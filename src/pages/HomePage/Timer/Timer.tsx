...
import {
	...
	increaseStatStopCount,
	increaseStatWorkSec,
} from '../../../store/slice/statSlice';
...
export function Timer() {
	...
	const [breakInMin, setBreakInMin] = useState(
		breaksCounter % 4 ? smallBreakTime : largeBreakTime
	);
	...
	useEffect(() => {
		let timerId = setInterval(() => {
			//Если запущен таймер
			if (
				(isStarted && !isPaused && timerInSeconds > 0) ||
				(isBreakStarted && !isBreakPaused && timerInSeconds > 0)
			) {
