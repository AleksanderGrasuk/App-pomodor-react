...
export function StatChart({
	...
	useEffect(() => {
		...
		for (let dayNumber = 0; dayNumber < 7; dayNumber++) {
			let weekDayDate = moment()
				.subtract(subtractDays, 'days')
				.weekday(dayNumber)
				.format('YYYY-MM-DD');
			let weekDayName = moment()
				.subtract(subtractDays, 'days')
				.weekday(dayNumber)
				.format('ddd');
			let workSec = 0;
		...
		maxWorkSec = weekdays.reduce(
			(prev, current) => (prev > current.workSec ? prev : current.workSec),
			0
		);
		...
		<div className={styles.chart}>
			<div className={styles.grid}>
				<div className={styles.line}>
					<span className={styles.time}>{secToTime(maxWorkSec)}</span>
				</div>
				<div className={styles.line}>
					<span className={styles.time}>{secToTime((maxWorkSec / 4) * 3)}</span>
				</div>
				<div className={styles.line}>
					<span className={styles.time}>{secToTime((maxWorkSec / 4) * 2)}</span>
				</div>
				<div className={styles.line}>
					<span className={styles.time}>{secToTime(maxWorkSec / 4)}</span>
				</div>
			</div>
			<div className={styles.weekday}>
				{weekdays.map((item) => {
					let className = classNames(
						styles.class,
						item.active
							? calcHeight(item.workSec, maxWorkSec) > 5 && styles.red
							: calcHeight(item.workSec, maxWorkSec) > 5 && styles.redLight
					);
					...
					return (
						<div
							...
							className={styles.weekdayTransfer}
						>
							<span className={text}>{item.name}</span>
							<div
								className={className}
								style={{ height: calcHeight(item.workSec, maxWorkSec) + 'px' }}
							/>
						</div>
					);
				})}
