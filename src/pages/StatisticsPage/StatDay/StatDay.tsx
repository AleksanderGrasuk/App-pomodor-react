...
export function StatDay({ selectedDayName, workInSec }: StatPomodoroProps) {
	...
	const hourMin = `${hour} ${declinationOfNumber(hour, [
		'часа',
		'часов',
		'часов',
	])} ${min} ${declinationOfNumber(min, ['минуты', 'минут', 'минут'])}`;
	const minSec = `${min} ${declinationOfNumber(min, [
		'минуты',
		'минут',
		'минут',
	])} ${sec} ${declinationOfNumber(sec, ['секунды', 'секунд', 'секунд'])}`;
